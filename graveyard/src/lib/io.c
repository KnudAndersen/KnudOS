#ifndef IO_C
#define IO_C

#include "./include/io.h"

void KERNEL_LOG(char *const msg)
{
	extern tty_t ttys[];
	tprints(&ttys[0], msg);
	return;
}

inline void tbufferc(tty_t *term, char c)
{
	if (term == 0) {
		while (1) {
			asm volatile("hlt");
		}
	}
	if (c == '\t') {
		for (uint8_t i = 0; i < TTY_TAB_WIDTH; i++)
			tprintc(term, ' ');
		return;
	}
	if (term->col_i == term->cols - 1 || c == '\n') {
		term->col_i = 1;
		term->row_i++;
		if (term->row_i == term->rows - 1) {
			tty_scroll(term, TTY_SCROLL_UP, 1);
			term->row_i = term->rows - 2;
		}
	}
	if (c != '\n' && c != '\t') {
		term->data[term->row_i][term->col_i++] =
			vga_get_attr(c, term->default_fg, term->default_bg);
	}
}

void tprintc(tty_t *term, char c)
{
	tbufferc(term, c);
	vga_tty_render(term);
}
void tbuffers(tty_t *term, const char *const str)
{
	uint32_t i = 0;
	while (str[i]) {
		tbufferc(term, str[i]);
		i++;
	}
}

void tprints(tty_t *term, char *str)
{
	tbuffers(term, str);
	vga_tty_render(term);
}
static inline void tbuffer_dec(tty_t *term, uint64_t val, uint8_t shift,
			       uint8_t mod, uint8_t size)
{
	if (val == 0) {
		tbufferc(term, '0');
		return;
	}
	char out[2 * size + 1];
	out[2 * size] = '\0';
	int32_t i = 2 * size - 1;
	while (val) {
		out[i] = HEXDIGITS[val % mod];
		i--;
		val /= shift;
	}
	tbuffers(term, out + (i + 1));
}
static inline void tprint_dec(tty_t *term, uint64_t val, uint8_t shift,
			      uint8_t mod, uint8_t size)
{
	if (val == 0) {
		tbufferc(term, '0');
		return;
	}
	tbuffer_dec(term, val, shift, mod, size);
	vga_tty_render(term);
}
static inline void tprint_pow2(tty_t *term, uint64_t val, uint8_t shift,
			       uint8_t mod, uint8_t size)
{
	if (val == 0) {
		KERNEL_LOG("0");
		return;
	}
	char out[2 * size + 1];
	out[2 * size] = '\0';
	int32_t i = 2 * size - 1;
	while (val) {
		out[i] = HEXDIGITS[val & mod];
		i--;
		val >>= shift;
	}
	tprints(term, out + (i + 1));
}

void tprintb(tty_t *term, uint8_t val)
{
	tprint_dec(term, val, DECMOD, DECMOD, sizeof(val));
}

void tbufferd(tty_t *term, uint32_t val)
{
	tbuffer_dec(term, val, DECMOD, DECMOD, sizeof(val));
}
void tprintd(tty_t *term, uint32_t val)
{
	tbuffer_dec(term, val, DECMOD, DECMOD, sizeof(val));
	vga_tty_render(term);
}

void tprintl(tty_t *term, uint64_t val)
{
	tprint_dec(term, val, DECMOD, DECMOD, sizeof(val));
}

void tprintbx(tty_t *term, uint8_t val)
{
	tprint_pow2(term, val, HEXSHIFT, HEXMOD, sizeof(val));
}

void tprintdx(tty_t *term, uint32_t val)
{
	tprint_pow2(term, val, HEXSHIFT, HEXMOD, sizeof(val));
}

void tprintlx(tty_t *term, uint64_t val)
{
	tprint_pow2(term, val, HEXSHIFT, HEXMOD, sizeof(val));
}

#endif
