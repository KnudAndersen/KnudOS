...
multiboot_mod *get_kernel(multiboot_info *m_info)
{
	...
	for (uint32_t i = 0; i < m_info->mods_count; i++) {
		...
		checksum = StrNCmp(boot_module_magic, mod_str,
				   sizeof(boot_module_magic) - 1);
		if (checksum == 0) {
			return mod_iter;
		}
		mod_iter++;
...
uint32_t pop_reg32(uint8_t reg)
{
	...
	switch (reg) {
	case ESI:
		asm volatile("mov %0, esi" : : "m"(ret) : "esi");
		break;
	case EDI:
		asm volatile("mov %0, edi" : : "m"(ret) : "edi");
		break;

	case EBX:
		asm volatile("mov %0, ebx" : : "m"(ret) : "ebx");
		break;
	default:
		break;
	}
