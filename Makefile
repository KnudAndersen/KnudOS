...
CFLAGS_64:=$(INCLUDE_64) -mcmodel=large -Wall -Wextra \
		-ffreestanding -fno-pic -fno-pie -masm=intel -gdwarf-4
...
SRC_32_ALL = $(shell find $(SRC_32_ROOT) \
		-type f $(foreach tmp,$(SRC_32_RAW_EXTNS),-name '*.$(tmp)' -or) \
		-not -name '*')
SRC_64_ALL = $(shell find $(SRC_64_ROOT) \
		-type f $(foreach tmp, $(SRC_64_RAW_EXTNS),-name '*.$(tmp)' -or) \
		-not -name '*')
